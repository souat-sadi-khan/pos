        ...
        <div class="row mb-2">
            ...
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    ...
                    <li class="breadcrumb-item active">{{ $model->customer_name }}</li>
                </ol>
            </div>
        </div>
    </div>
...
<div class="col-md-12">
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Customer Full View Information :: {{ $model->customer_name }}</h3>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-4 card card-widget widget-user">
                    <div class="widget-user-header bg-info">
                        <p class="widget-user-username">{{ $model->customer_name }}</p>
                    ...
                    <div class="card-footer">
                        <div class="row">
                            ...
                            <div class="col-sm-4">
                                <div class="description-block">
                                    <div class="description-block">
                                        <a role="button" id="content_managment" data-url="{{ route('admin.customer.edit',$model->id) }}" class="btn btn-block btn-sm btn-warning" href="" title="Edit Customer">
                    ...
                    <div class="card-body px-0">
                        <div class="bg-light p-1">
                            @if ($model->customer_mobile != '')
                                <h4 class="text-center">Mobile : {{ $model->customer_mobile }} </h4>
                            @endif
                            @if ($model->customer_email != '')
                                <h6 class="text-center">Email : {{ $model->customer_email }} </h6>
                            @endif
                            @if ($model->customer_address != '')
                                <h6 class="text-center">Address : {{ $model->customer_address }} </h6>
                            @endif
                            @if ($model->customer_city != '')
                                <h6 class="text-center">City : {{ $model->customer_city }} </h6>
                            @endif
                            @if ($model->customer_state != '')
                                <h6 class="text-center">State : {{ $model->customer_state }} </h6>
                            @endif
                            @if ($model->customer_country != '')
                                <h6 class="text-center">Country : {{ $model->customer_country }} </h6>
                            @endif
                        </div>    
                    </div>
                </div>
